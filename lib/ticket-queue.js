...
const { VALID_STATUSES } = require('./ticket-lanes');
...
// system lifecycle todo → defining → in-progress → testing → done, PLUS the
// claimable fix-loop status failed-testing (TASK-206 removed post-processing
// from this lifecycle). This mirrors (and is derived from)
// lib/ticket-lanes.js VALID_STATUSES so the persistable-status enum and the
// swarm-owned set can never drift apart.
//
...
function claimTicket(fm, agentId, opts) {
  ...
  // (no-agent-id → claimed → not-claimable) is unchanged.
...
//   no-ticket → claimed → already-active → not-claimable → no-slots → ok
