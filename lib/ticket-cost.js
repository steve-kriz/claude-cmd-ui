...
// `review`, `post-processing`, plus any future activity string. Each time the
...
// tokensOut, costUsd } entry, so a ticket accumulates a complete cost view over
// its lifetime.
//
...
//     isValidAmount (a finite number >= 0); missing / NaN / negative / '' values
//     leave the field absent. Note isValidAmount(0) is true — 0 is a valid
//     recorded amount and is distinct from "absent".
...
// - `tokensIn` / `tokensOut` / `costUsd` recorded only when a finite number >= 0
//                (isValidAmount); missing/invalid values leave the field absent.
...
// field, so a missing category is never reported as a fabricated 0 and no total is
// ever NaN.
