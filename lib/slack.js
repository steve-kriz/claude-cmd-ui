...
// Pull replies within a single thread newer than `oldest` (a Slack ts string).
// This is REQUIRED for the two-way proxy: conversations.history returns only
// top-level channel messages and never the replies posted inside a thread, so a
// user's reply in the session anchor thread is invisible to history polling.
// conversations.replies is the only way to see them. `ts` is the thread's parent
// (the anchor's ts). Returns oldest-first, parent included, so callers should
// skip the parent (or pass oldest to exclude already-seen replies). Uses the
// same channels:history / groups:history scopes as fetchHistory.
async function fetchReplies(token, channel, ts, oldest, limit) {
  if (!ts) return { ok: true, messages: [] };
  const params = { channel, ts, limit: Math.max(1, Math.min(200, Number(limit) || 200)) };
  if (oldest) { params.oldest = oldest; params.inclusive = false; }
  const res = await slackRequest('conversations.replies', token, params, true);
  if (!res.ok) return { ok: false, error: describeError(res, 'conversations.replies failed') };
  const messages = (res.messages || [])
    .slice()
    .sort((a, b) => Number(a.ts) - Number(b.ts));
  return { ok: true, messages };
...
// Exchange an OAuth authorization code for tokens via oauth.v2.access. Unlike
// every other call here this endpoint is UNauthenticated (no bearer token) and
// expects application/x-www-form-urlencoded with the client id/secret in the
// body — so it can't go through slackRequest (which posts JSON with a Bearer
// header). Returns Slack's raw JSON: { ok, authed_user: { access_token, id, … },
// team: { id, name }, … } on success, or { ok:false, error } on failure.
function exchangeOAuthCode({ clientId, clientSecret, code, redirectUri } = {}) {
  return new Promise((resolve) => {
    if (!clientId || !clientSecret) {
      return resolve({ ok: false, error: 'SLACK_CLIENT_ID and SLACK_CLIENT_SECRET are required.' });
    }
    if (!code) return resolve({ ok: false, error: 'Missing authorization code.' });
    ...
    const req = https.request(
      { hostname: SLACK_API, path: '/api/oauth.v2.access', method: 'POST', headers },
      (res) => {
        let data = '';
        res.on('data', (chunk) => { data += chunk; });
        res.on('end', () => {
          ...
          if (!json.ok) json.error = describeError(json, 'oauth.v2.access failed');
          resolve(json);
        });
      }
    );
...
module.exports = { authTest, resolveChannel, connect, fetchHistory, fetchReplies, postMessage, openSocketUrl, exchangeOAuthCode };
