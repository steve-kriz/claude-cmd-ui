...
// `/usage` now opens an interactive TABBED dialog (observed tabs: Settings /
// Status / Config / Session) rather than printing the weekly row directly, and
// the tab it OPENS ON is a session cost/duration summary — not the one with the
// weekly percentage. "Esc to cancel"/"Esc to close" is the one hint every tab of
// that dialog shows (and the plain booted composer never does), so it is used
// below as a version/tab-layout-agnostic "a modal is open" signal to decide when
// it's safe to nudge through the other tabs looking for the weekly row.
const DIALOG_OPEN_RE = /Esc\s*to\s*\S/i;
...
const DEFAULTS = {
  ...
  settleMs: 1200,
  // Gap between tab-cycle nudges (below) — long enough for the dialog to repaint
  // after a keypress — and how many tabs to try before giving up and letting the
  // normal unparsed/timeout path report the failure. Bounded so a plan/version
  // whose dialog has no weekly-figure tab at all (nothing to find) still exits
  // via the existing ceiling rather than spinning.
  tabCycleMs: 900,
  maxTabCycles: 4,
  ...
  return new Promise((resolve) => {
    ...
    const askForUsage = () => {
      ...
      later(() => {
        write('\r');
        phase = 'reading';
        // The dialog `/usage` now opens defaults to a tab with no weekly row
        // (see DIALOG_OPEN_RE above). Nudge through the other tabs with the
        // same Left arrow a person would press, re-checking for the weekly row
        // after each — bounded, and a no-op the instant the row is found (the
        // `onData` handler below settles as soon as FRAME_READY_RE matches, so
        // these nudges normally never fire at all for a dialog that opens
        // straight on the right tab).
        for (let i = 1; i <= o.maxTabCycles; i++) {
          later(() => {
            if (settled || phase !== 'reading') return;
            const clean = buf.replace(ANSI_REGEX, '');
            if (FRAME_READY_RE.test(clean)) return;
            if (DIALOG_OPEN_RE.test(clean)) write('\x1b[D');
          }, o.tabCycleMs * i);
        }
...
module.exports = {
  probeUsage,
  __testing: { ANSI_REGEX, READY_RE, FRAME_READY_RE, TRUST_PROMPT_RE, DIALOG_OPEN_RE, DEFAULTS },
};
