...
// the premium `claude-opus-4-8` tier, falling back to the swarm default
// `claude-sonnet-5` if that is unavailable. Used as the canonical value when the
// prose form is present but its exact model tokens cannot be teased apart.
const PLAN_MODEL_PRIMARY = 'claude-opus-4-8';
const PLAN_MODEL_FALLBACK = 'claude-sonnet-5';
...
// `claude-opus-4-8`; when present, returns { primary, fallback } capturing the
// stated fallback (`claude-sonnet-5`, the swarm default, when that token also
// appears, else the canonical fallback). Returns null when no directive is stated.
function modelDirectiveIn(text) {
  ...
  // is not the primary (SKILL.md's Model-routing directive names the premium
  // `claude-opus-4-8` planning tier and the default `claude-sonnet-5` it degrades
  // to). Degrade to the canonical fallback if absent.
  let fallback = null;
...
module.exports = {
  ...
  PLAN_MODEL_FALLBACK,
  parseWorkflow,
  // Exported for lib/skill-section.js (TASK-184), which reuses this exact
  // fence-aware level-2 splitter + phase-number matcher to scope a section
  // splice to one `## Phase <n>` body without re-implementing the logic.
  sectionsOf,
  phaseNumberOf
};
