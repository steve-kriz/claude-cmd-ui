...
const { LANE_STATUSES, ACTIVE_STATUSES, FAILED_STATUS, laneForStatus } = require('./ticket-lanes');
...
// frontmatter objects — both are tolerated. Frontmatter is authoritative. The
...
//       "Nothing is being worked on right now." when none are active.
//   *Failed testing:*        — same line shape, only shown when at least one
//       failed-testing ticket exists.
//   lane counts              — LANE_STATUSES order, "todo 3 · defining 1 · …";
//       failed-testing folds into `testing` (via laneForStatus) and any
//       out-of-enum status is counted under a trailing "unknown N".
// Never throws: an empty/null/non-array input returns "The tasks board is
...
function formatTasksSummary(tickets) {
  ...
  // Lane counts: seed every lane at 0 (stable columns), fold failed-testing into
  // testing and route out-of-enum statuses to "unknown" via laneForStatus.
  const counts = new Map(LANE_STATUSES.map((s) => [s, 0]));
  let unknown = 0;
  for (const fm of fms) {
    const lane = laneForStatus(fm.status);
  ...
  const countPieces = LANE_STATUSES.map((s) => `${s} ${counts.get(s)}`);
