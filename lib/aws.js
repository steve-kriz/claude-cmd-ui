...
// ── Claude variables: the shared config secret → .env ───────────────────────
//
// One AWS Secrets Manager secret holds the configuration this app needs
// (Slack/Atlassian/Anthropic credentials and friends) as a flat JSON object.
// `syncClaudeVariables` signs in to the DEV account, reads that secret, and
// returns the key/value pairs; main.js is what persists them into `.env`. Once
// they land there, every existing consumer (getSlackToken, the Atlassian
// helpers, …) picks them up unchanged — .env stays the single source of truth
// and nothing else in the app has to learn about Secrets Manager.

const CLAUDE_SECRET_DEFAULT_ID = '/dev/claude-cmd-ui';

// Overridable so the same button can target another environment's secret.
function claudeSecretId() {
  return (process.env.CLAUDE_SECRET_ID || '').trim() || CLAUDE_SECRET_DEFAULT_ID;
}

// A profile used ONLY for reading the secret. Deliberately not one of the
// `sso-<account>` profiles the environment switcher manages: syncing variables
// must not repoint [default], rewrite ~/.aws/credentials, or change the role on
// a profile the user is actively working in.
const SECRETS_PROFILE_NAME = 'claude-cmd-ui-secrets';

// Which discovered account is "dev". AWS_DEV_ACCOUNT_ID wins when set; the name
// match is the fallback so this works before that variable is configured.
function pickDevAccount(accounts) {
  const list = Array.isArray(accounts) ? accounts : [];
  const configured = devAccountId();
  if (configured) return list.find((a) => a.accountId === configured) || null;
  return list.find((a) => /(^|[^a-z])dev([^a-z]|$)/i.test(a.accountName || '')) || null;
}

// Secret keys are written by humans ("slack token", "slackToken", "Slack-Token")
// but .env keys are SCREAMING_SNAKE. Normalize so any of those spellings lands
// on SLACK_TOKEN. Returns '' when nothing usable is left.
function envKeyFromSecretKey(key) {
  const s = String(key == null ? '' : key)
    ...
    .toUpperCase();
  if (!s) return '';
  // env-store requires a leading letter or underscore.
  return /^[0-9]/.test(s) ? `_${s}` : s;
}

// Pure: the secret's JSON string → { vars, skipped }. Every key in the secret
// that can be represented in .env comes back in `vars` — including ones with an
// empty value, so the secret's full key list always lands in .env even for keys
// nobody has filled in yet. Whether an empty value may overwrite an existing
// local value is NOT decided here; main.js applies that rule when it writes.
// Every rejection is reported rather than dropped, so the UI can say what did
// NOT make it across.
function envVarsFromSecret(secretString) {
  let parsed;
  try {
    parsed = JSON.parse(secretString);
  } catch (e) {
    throw new Error(`Secret is not valid JSON: ${e.message}`);
  }
  ...
  for (const [rawKey, rawVal] of Object.entries(parsed)) {
    const key = envKeyFromSecretKey(rawKey);
    if (!key) {
      skipped.push({ key: rawKey, reason: 'not a usable .env key' });
      continue;
    }
    if (Object.prototype.hasOwnProperty.call(vars, key)) {
      skipped.push({ key: rawKey, reason: `collides with an earlier key on ${key}` });
      continue;
    }
    if (rawVal == null) {
      skipped.push({ key: rawKey, reason: 'value is null' });
      continue;
    }
    const value = typeof rawVal === 'object' ? JSON.stringify(rawVal) : String(rawVal);
    // .env is line-based — env-store's parser reads one key per line — so a
    // value containing a newline cannot round-trip. Refuse it rather than
    // write a file that reads back wrong.
    if (/[\r\n]/.test(value)) {
      skipped.push({ key: rawKey, reason: 'value spans multiple lines' });
      continue;
    }
    vars[key] = value;
  }
...
// Ensure the read-only secrets profile points at the dev account with `role`.
async function ensureSecretsProfile(acc, role) {
  const content = await readTextSafe(CONFIG_PATH);
  const updated = upsertAccountProfileText(content, SECRETS_PROFILE_NAME, acc.accountId, role);
  await fsp.mkdir(AWS_DIR, { recursive: true });
  await atomicWrite(CONFIG_PATH, updated);
  return SECRETS_PROFILE_NAME;
}

function getSecretString(secretId, profileName) {
  return new Promise((resolve, reject) => {
    execFile(
      AWS_EXE,
      [
      ...
      (err, stdout, stderr) => {
        ...
        if (!out || out === 'None') return reject(new Error(`Secret ${secretId} has no SecretString value.`));
        resolve(out);
      }
    );
  });
...
// Sign in to dev, read the secret, and return its key/value pairs. Does NOT
// touch ~/.aws/credentials or the active environment — reading config should
// not hijack whichever account the user is currently working in.
async function syncClaudeVariables(onLine) {
  if (inflight) {
    if (onLine) onLine('[claude-cmd-ui] another aws operation is in progress; waiting...');
    try { await inflight; } catch (_) { /* swallow */ }
  }
  const run = (async () => {
    const secretId = claudeSecretId();
    const token = await getValidSsoToken(onLine);
    const accounts = await ssoListAccounts(token);
    const acc = pickDevAccount(accounts);
    if (!acc) {
      const names = accounts.map((a) => a.accountName).join(', ') || '(none)';
      throw new Error(
        `Could not identify the dev account among: ${names}. ` +
        'Set AWS_DEV_ACCOUNT_ID in .env to name it explicitly.'
      );
    }
    const roles = await listAccountRoles(acc.accountId, token);
    const role = roles.includes(PLACEHOLDER_ROLE) ? PLACEHOLDER_ROLE : roles[0];
    if (!role) throw new Error(`No roles are available to you in ${acc.accountName}.`);

    if (onLine) onLine(`[claude-cmd-ui] reading ${secretId} from ${acc.accountName} as ${role}...`);
    const profile = await ensureSecretsProfile(acc, role);
    const secretString = await getSecretString(secretId, profile);
    const { vars, skipped } = envVarsFromSecret(secretString);
    if (onLine) {
      onLine(`[claude-cmd-ui] secret has ${Object.keys(vars).length} usable variable(s): ${Object.keys(vars).join(', ') || '(none)'}`);
      for (const s of skipped) onLine(`[claude-cmd-ui] skipped "${s.key}" — ${s.reason}`);
    }
    return { secretId, accountId: acc.accountId, accountName: acc.accountName, role, vars, skipped };
  })();
  inflight = run;
  try { return await run; }
  finally { if (inflight === run) inflight = null; }
...
module.exports = {
  ...
  claudeSecretId,
  __testing: {
    rewriteCredentialsText,
    resolveAwsExe,
    normalizeSsoUrl,
    upsertAccountProfileText,
    envKeyFromSecretKey,
    envVarsFromSecret,
    pickDevAccount,
    SECRETS_PROFILE_NAME
  }
};
