...
const AWS_EXE = 'C:\\Program Files\\Amazon\\AWSCLIV2\\aws.exe';
...
async function ensureAccountProfile(acc, role) {
  ...
  // Profile already exists — just refresh the selected role.
  const lines = content.split(/\r?\n/);
  const section = findProfileSection(lines, name);
  let roleSet = false;
  for (let i = section.start + 1; i < section.end; i++) {
    if (/^\s*sso_role_name\s*=/.test(lines[i])) {
      lines[i] = `sso_role_name = ${role}`;
      roleSet = true;
      break;
    }
  }
  if (!roleSet) lines.splice(section.start + 1, 0, `sso_role_name = ${role}`);
  await atomicWrite(CONFIG_PATH, lines.join('\n'));
  return name;
}
...
module.exports = {
  ...
  setUserDataDir,
  getSlackToken,
  __testing: { rewriteCredentialsText }
};
