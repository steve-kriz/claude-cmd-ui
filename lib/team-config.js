...
//         "agent": null, "system": true, "phase": null },
//       { "status": "ux-review", "label": "UX Review", "description": "...",
//         "agent": "orchestrate-tech-lead", "system": false, "phase": "review" } ],
...
//
// skill.phases (TASK-180) is the schema backing the Workflow panel's per-phase
// enable/reorder (TASK-182) and the live phase-skipping behaviour (TASK-181).
// A column's `phase` is optional metadata linking it to one of the four
// canonical phase keys (or null), backing the Board panel's column->phase link
// (TASK-183). Both are normalised — never interpreted — by this module; see
// normalizePhases / normalizeColumnPhase below.
//
// Columns
// -------
...
const { PHASE_SPECS } = require('./skill-workflow');
...
const COLUMN_KEYS = ['status', 'label', 'description', 'agent', 'system', 'phase'];

// The four canonical orchestrate phase keys, in canonical order — sourced from
// PHASE_SPECS (lib/skill-workflow.js) rather than hardcoded here, so this
// module's phase set can never drift from SKILL.md's own Phase 1-4 order
// (TASK-180). Backs both skill.phases (enabled/order per phase) and a column's
// optional `phase` link.
const PHASE_KEYS = PHASE_SPECS.map((spec) => spec.key);

// Default { enabled, order } per phase. plan/build/test mirror their historic
// always-on, implicit system-column correspondence; review is opt-in by
// default (enabled: false) since, unlike the other three, it has no system
// board column today — a project must explicitly link a column to `review`
// (TASK-183) or flip it on (TASK-182) before it participates.
const PHASE_DEFAULTS = Object.create(null);
for (const spec of PHASE_SPECS) {
  PHASE_DEFAULTS[spec.key] = { enabled: spec.key !== 'review', order: spec.number };
}

// A fresh skill.phases object: all four canonical keys at their defaults.
function defaultPhases() {
  const out = {};
  for (const key of PHASE_KEYS) {
    const def = PHASE_DEFAULTS[key];
    out[key] = { enabled: def.enabled, order: def.order };
  }
  return out;
}
...
// Normalise a column's `phase` link (TASK-180): a string exactly equal to one
// of the four canonical phase keys is kept; anything else (missing, unknown,
// non-string) becomes null. A phase link is optional metadata, so a warning is
// only recorded when a non-null, non-valid value was actually present.
function normalizeColumnPhase(rawPhase, warnings, slug) {
  if (rawPhase == null) return null;
  if (typeof rawPhase === 'string' && PHASE_KEYS.includes(rawPhase)) return rawPhase;
  if (warnings) {
    warnings.push(`column "${slug || '(blank)'}" had an invalid phase ${JSON.stringify(rawPhase)}; reset to null`);
  }
  return null;
}
...
// Build a fresh, canonical system column for `slug`. `phase` starts null — no
// column is auto-linked to a phase (TASK-180); linkage is user-driven (TASK-183).
...
function repairSystemColumn(slug, rawCol, warnings) {
  ...
  src.phase = normalizeColumnPhase(src.phase, warnings, slug);
...
function buildUserColumn(rawCol, warnings) {
  ...
  if (typeof src.description !== 'string') src.description = '';
  src.agent = normalizeAgent(src.agent);
  src.phase = normalizeColumnPhase(src.phase, warnings, src.status);
  return orderColumn(src);
}
...
function defaultConfig() {
  return {
    ...
    skill: { concurrencyDefault: DEFAULT_CONCURRENCY, phases: defaultPhases() },
  };
}

// Normalise skill.phases (TASK-180) into an object with EXACTLY the four
// canonical PHASE_KEYS. Rules (never throws; every repair is warned):
//   - the whole value missing/not-an-object → all four phases at defaults
//     (a warning per re-inserted phase, only when a `phases` object WAS
//     present but a key was missing from it — a wholesale reset already
//     covers every key with its own defaulting path below).
//   - a present phase value that is not an object → that phase's defaults.
//   - `enabled`: strict boolean kept; missing OR non-boolean → true (a
//     warning is recorded only when a non-boolean value was actually present).
//   - `order`: coerced via Number(); a positive integer kept; missing/
//     non-integer/<=0 → that phase's canonical default order, with a warning.
//   - any key beyond the four canonical ones is dropped, with a warning
//     naming it (prototype-poisoning keys are dropped silently-but-noted via
//     the same "unsafe" warning style used elsewhere in this module).
// Order COLLISIONS across phases are permitted here — this module only
// coerces each phase's own value; the panel/behaviour tickets own tie-breaks.
function normalizePhases(rawPhases, warnings) {
  const providedIsObject = isPlainObject(rawPhases);
  if (rawPhases != null && !providedIsObject) {
    warnings.push('skill.phases was not an object; reset to defaults');
  }
  const src = providedIsObject ? rawPhases : null;
  const out = {};

  for (const key of PHASE_KEYS) {
    const def = PHASE_DEFAULTS[key];
    const hasKey = !!src && Object.prototype.hasOwnProperty.call(src, key);
    if (!hasKey) {
      if (src) warnings.push(`missing phase "${key}" re-inserted with defaults`);
      out[key] = { enabled: def.enabled, order: def.order };
      continue;
    }

    const rawPhase = src[key];
    const rp = isPlainObject(rawPhase) ? rawPhase : null;
    if (!rp) {
      warnings.push(`phase "${key}" was not an object; reset to defaults`);
      out[key] = { enabled: def.enabled, order: def.order };
      continue;
    }
    ...
    if (typeof rp.enabled === 'boolean') {
      enabled = rp.enabled;
    } else if (rp.enabled === undefined) {
      enabled = true;
    } else {
      warnings.push(`phase "${key}".enabled was not a boolean; reset to true`);
      enabled = true;
    }

    let order;
    const rawOrder = rp.order;
    const n = Number(rawOrder);
    if (rawOrder !== undefined && Number.isInteger(n) && n > 0) {
      order = n;
    } else {
      warnings.push(`phase "${key}".order was invalid; reset to ${def.order}`);
      order = def.order;
    }
  ...
  if (src) {
    for (const k of Object.keys(src)) {
      if (PHASE_KEYS.includes(k)) continue;
      if (isUnsafeKey(k)) {
        warnings.push(`ignored unsafe phase key "${k}"`);
        continue;
      }
      warnings.push(`unknown phase "${k}" dropped`);
    }
  }
...
function normalizeConfig(raw) {
  ...
  try {
    ...
    // skill.concurrencyDefault clamped through resolveConcurrency; skill.phases
    // normalised through normalizePhases (TASK-180); unknown skill fields
    // round-trip.
    ...
    const phases = normalizePhases(rawSkill.phases, warnings);
    const skill = { concurrencyDefault: resolvedConc, phases };
    for (const k of Object.keys(rawSkill)) {
      if (k === 'concurrencyDefault' || k === 'phases') continue;
      if (isUnsafeKey(k)) {
