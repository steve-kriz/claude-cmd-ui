...
// agent how to handle tickets passing through that step. The two PASSIVE
// columns (`todo`, `done`) carry no agent by design, so their text documents
// that fact for the reader rather than instructing a dispatch: giving `done` an
// agent would re-dispatch every finished ticket on each /orchestrate build.
const SYSTEM_COLUMN_DEFAULT_INSTRUCTIONS = Object.freeze({
  ...
  defining: 'Define the ticket before any code is written. First read and search the relevant parts of the codebase so the definition is grounded in what actually exists, then write into the ticket body: a precise Description, complete Acceptance Criteria, Cucumber Tests (Gherkin, at least one failure/edge scenario), the explicit edge and failure cases, and the files/context the implementer will need. Never touch the user-owned Additional Context. If the ticket is already defined (real Acceptance Criteria plus a non-empty gherkin block), pass it through unchanged without redefining. Raise any clarifying question on the ticket\'s question field rather than guessing. The Acceptance Criteria you write are FROZEN once the ticket leaves this column and are the ONLY basis on which a later column may fail the work, so write each one as a testable, binary statement rather than a description — if you cannot state it that way, raise it as a clarifying question instead. Define only — never write implementation code.',
  'in-progress': 'Implement the ticket to its Acceptance Criteria inside this ticket\'s isolated worktree/branch. Honour the Additional Context. Follow the conventions of the surrounding code rather than introducing a new style. Keep the change scoped to this ticket — no drive-by refactors of unrelated code. Do not write tests beyond what compilation/wiring needs; the Testing column owns tests. Report the files you changed and anything the tester should know.',
  testing: 'Write BOTH e2e cucumber-style node --test scenarios AND unit tests covering every Acceptance Criterion including at least one failure/edge path. Mock all database calls — never open a real connection. Run the full suite under node --test and report pass/fail with the failure output verbatim. A red result returns the ticket for a fix, capped at 3 attempts. Judge the implementation on two grounds only: does it satisfy the STATED Acceptance Criteria (frozen at Defining), and does it break something that previously worked. Return at most 3 findings, force ranked by severity — fewer is fine, and zero findings is a normal, expected outcome. Style, naming, code organisation and hypothetical future refactors are out of scope unless they cause a correctness or security problem you can name. Anything else is an observation that expires when the ticket reaches Done: never create a ticket for it.',
...
// The starter board's one USER column (see starterConfig): a review lane sitting
// between `testing` and `done`, wired to the bundled tech-lead agent. It is NOT
// part of defaultConfig — defaultConfig is also the repair baseline normalizeConfig
// falls back to, and re-injecting a user column there would resurrect it every time
// a user deleted it. Seeded once, at install, into a project that has no board
// config yet; from then on it is an ordinary user column the user may edit or remove.
const STARTER_REVIEW_COLUMN = Object.freeze({
  status: 'pr-review',
  label: 'PR Review',
  description: 'Tech-lead review of the finished implementation before it reaches Done.',
  agent: AGENT_TYPES.techLead,
  instructions: 'Triage gate, not a fifth pair of eyes: your output is a SMALLER list than your input, and you are scored on how much you discard. Read BOTH the ticket and the code it produced. Judge on two grounds only — does the implementation satisfy the STATED Acceptance Criteria (frozen at Defining), and does it break something that previously worked. Security defects in the changed code are always in scope (input validation, authorisation, injection, secrets never logged or committed), and a test that never exercises the new code path means the criterion it claims to cover is not satisfied. Discard anything you cannot trace to a stated criterion, a demonstrable regression or a concrete security defect — style, naming, code organisation and hypothetical future refactors are out of scope — and report how many findings you discarded and why. Return at most 3 findings, force ranked by severity: fewer is better, and zero is a normal, expected outcome. Issue a reject-and-rework verdict — sending the ticket back to Testing — ONLY for a critical finding that must be fixed before this ticket can proceed. Every other survivor becomes a DEFERRED follow-up To Do ticket carrying a review-of key naming the reviewed ticket, never built in the same cycle; everything discarded is an observation that expires when the ticket reaches Done. Leave the reviewed ticket\'s status and frontmatter untouched. Review only — never edit the code yourself, and never create tickets.',
  system: false,
});
...
// The STARTER config: what a brand-new project's tasks/team-config.json is
// seeded with when the orchestrate skill is installed (main.js's
// `tasks:installSkill`, which writes it only when no config exists yet). It is
// defaultConfig plus STARTER_REVIEW_COLUMN inserted before `done`, so an
// out-of-the-box board dispatches all four bundled agents —
// BA -> coder -> tester -> tech-lead — with no manual column setup.
//
// Deliberately SEPARATE from defaultConfig: defaultConfig doubles as the repair
// baseline normalizeConfig falls back to, so a user column seeded there would be
// re-injected (resurrected) every time the user deleted it. Seeding happens once,
// at install; after that the review column is an ordinary, deletable user column.
function starterConfig() {
  const cfg = defaultConfig();
  const doneIdx = cfg.columns.findIndex((c) => c.status === 'done');
  const review = orderColumn({ ...STARTER_REVIEW_COLUMN });
  // Insert before `done`; if `done` somehow isn't present, append.
  if (doneIdx === -1) cfg.columns.push(review);
  else cfg.columns.splice(doneIdx, 0, review);
  return cfg;
}
