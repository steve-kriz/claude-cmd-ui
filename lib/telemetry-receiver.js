...
function createTelemetryReceiver(opts = {}) {
  ...
  //
  // It also carries `projectRecent` — that same bucket's per-call rows, capped
  // exactly like getUsage()/getUsageForProject()'s `recent` — so
  // the Stats tab's per-prompt log (tokens up/down, model, cost) can render
  // live off ONE pushed payload instead of an extra getUsage IPC round-trip
  // after every ingest. Additive: existing consumers reading only
  // `usage`/`metricTotals`/`running`/`project`/`projectUsage` are unaffected.
  function snapshotState(project) {
    ...
    return {
      ...
      projectRecent: (bucket ? bucket.recent : []).slice(-100),
    };
  }
  ...
  // scoped to ONE project's own bucket — so a call captured for a different,
  // concurrently-running project (whose timestamp happens to fall inside the
  // same window) is NEVER folded into this project's prompt total. Mirrors
  // usageForProject's project-scoping pattern (get the one bucket, aggregate
  // only ITS rows) applied to usageForWindow's time-window correlation instead
  // of aggregateUsage's full-bucket rollup. Sums the bucket's FULL de-duplicated
  // store (not the capped `recent` feed) so an older prompt's window still
  // correlates correctly. Pure delegation — never throws, returns
  // emptyTotals() when the project has no bucket / no rows / nothing in the
  // bucket matches the window.
  //
  // TASK-198: the per-prompt correlation window is exclusive at the END —
  // [startedAt, finishedAt) — because `finishedAt` here is the NEXT prompt's
  // own `ts`, which is that next prompt's `startedAt`. A row timestamped
  // exactly `finishedAt` belongs to the next prompt's window, not this one;
  // `tel.usageForWindow` is inclusive at both ends (its intentional, unchanged
  // contract for TASK-142's per-ticket cost use above), so the exact-boundary
  // row is filtered out of the rows handed to it HERE, at this call site only,
  // rather than altering usageForWindow itself. This function is used solely
  // by the per-prompt correlation path (renderer's correlatePromptEntryUsage),
  // so no other caller is affected by this exclusive-end behavior.
  function usageForWindowInProject(project, window) {
    const bucket = getBucket(project);
    const rows = bucket ? Array.from(bucket.store.values()) : [];
    const win = window && typeof window === 'object' ? window : null;
    const finishedAt = win ? new Date(win.finishedAt).getTime() : NaN;
    const rowsExclusiveEnd = Number.isFinite(finishedAt)
      ? rows.filter((r) => {
          const ts = r && new Date(String(r.timestamp || '')).getTime();
          return !(Number.isFinite(ts) && ts === finishedAt);
        })
      : rows;
    return tel.usageForWindow(rowsExclusiveEnd, window);
  }
  ...
  return {
    ...
    usageForWindow, usageForWindowInProject, clear, otelEnv,
