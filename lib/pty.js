...
function spawnBash({ cwd, cols, rows, cliCommand }) {
  ...
  proc.onData((data) => {
    ...
    if (buf.length > 4096) buf = buf.slice(-4096);
    const clean = buf.replace(ANSI_REGEX, '');
    if (BASH_PROMPT_REGEX.test(clean)) {
      // 50 ms after the prompt renders so bash is ready to accept input
      setTimeout(launch, 50);
    }
  });

  // Fallback: bash buffers stdin, so the command still runs once the prompt appears.
  setTimeout(launch, CMD_AUTOLAUNCH_FALLBACK_MS);
...
function spawnShell({ shell, cwd, cols, rows, worker, cliCommand }) {
  ...
  if (shell === 'bash') return spawnBash({ cwd, cols, rows, cliCommand });
