...
// TASK-153: opts.env is an optional per-spawn overlay (e.g. a project-tagged
// OTEL_RESOURCE_ATTRIBUTES) merged LAST on top of the shell's own env. When
// absent/empty/not-an-object it is a no-op so existing callers are unaffected.
function hasEnvOverlay(env) {
  return !!env && typeof env === 'object' && Object.keys(env).length > 0;
}

// Spawn the POSIX login shell backing a pane on non-win32 platforms.
function spawnPosix({ cwd, cols, rows, env }, platform, ptyBackend) {
  ...
  return ptyBackend.spawn(shell, ['-l', '-i'], {
    ...
    env: {
      ...process.env,
      TERM: 'xterm-256color',
      ...(hasEnvOverlay(env) ? env : null)
    }
  });
}

function spawnCmd({ cwd, cols, rows, cliCommand, env }, platform, ptyBackend) {
  if (platform === 'win32') {
    const proc = ptyBackend.spawn('cmd.exe', [], {
      ...
      rows: rows || 30,
      cwd,
      env: hasEnvOverlay(env) ? { ...process.env, ...env } : process.env
    });
    if (cliCommand) autolaunch(proc, cliCommand, '\r', CMD_PROMPT_REGEX);
    return proc;
  }

  const proc = spawnPosix({ cwd, cols, rows, env }, platform, ptyBackend);
...
function spawnWorker({ worker, cwd, cols, rows, env }, platform, ptyBackend) {
...
function spawnBash({ cwd, cols, rows, cliCommand, env }, platform, ptyBackend) {
  if (platform === 'win32') {
