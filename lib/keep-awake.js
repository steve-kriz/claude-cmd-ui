...
//
// The keep-awake status set is the board's ACTIVE_STATUSES (defining /
// in-progress / testing — an agent is literally working the card) PLUS
// `post-processing`, whose final-event work (bug reports, accounting, done moves)
// is still live work the user is waiting on. It is derived from lib/ticket-lanes
// so it stays in lockstep with the canonical status enum rather than hardcoding
// strings.

const { ACTIVE_STATUSES, POST_PROCESSING_STATUS } = require('./ticket-lanes');

// Statuses that mean real work is running and the OS must not sleep.
const KEEP_AWAKE_STATUSES = [...ACTIVE_STATUSES, POST_PROCESSING_STATUS];
