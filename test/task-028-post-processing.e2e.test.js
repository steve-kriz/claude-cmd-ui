...
// TASK-101: the board lanes are now generated by renderTasksBoard from the team
// config (no longer static index.html columns), so the lane-order and Add-button
// scenarios drive the REAL renderer via this headless harness (mock DOM).
...
// TASK-101 made routing config-aware. The TASK-028 scenarios use no user columns,
// so the user-status set is empty here (behaviourally identical to before), but the
// copy keeps the `&& !USER_STATUSES.has(...)` term so it stays structurally
// identical to the real renderTasksBoard routing (DRIFT GUARD 1 compares them).
const USER_STATUSES = new Set();
function placeCard(fm) {
  const unknown = !VALID_STATUSES.includes(fm.status) && !USER_STATUSES.has(fm.status);
  let laneKey;
...
test('Scenario: the failed-testing lane is gone and post-processing takes its place', () => {
  // When the board renders its lanes. TASK-101: lanes are generated by the REAL
  // renderTasksBoard from the (default) team config, not scanned from index.html.
  ...
  const lanesGen = laneHarness.laneEls(tab);
  const laneOrder = lanesGen.map((el) => el.dataset.status);
  // Then there is no lane with data-status "failed-testing"
  assert.ok(!laneOrder.includes('failed-testing'), 'no failed-testing lane is generated');
  ...
  // And the post-processing lane header reads "Post-processing" with a count span
  // + an Add button (generated by buildTasksLaneEl).
  const ppLane = lanesGen.find((el) => el.dataset.status === 'post-processing');
  assert.equal(laneHarness.findByClass(ppLane, 'tasks-lane-label').textContent, 'Post-processing');
  assert.ok(laneHarness.findByClass(ppLane, 'tasks-lane-count'), 'a count span is present');
  assert.ok(laneHarness.findByClass(ppLane, 'tasks-lane-add'), 'an Add button is present');
});
...
test('Scenario: adding a post-processing ticket from the lane Add button writes status+kind into tasks/post-processing/', () => {
  // Given the post-processing lane Add button opens the modal in post-processing
  // mode. TASK-101: the Add affordance is now created inside buildTasksLaneEl (a
  // `<button class="tasks-lane-add">` with an inline click listener) rather than a
  // static index.html button wired by a `.tasks-lane-add` querySelector — so the
  // marker is the className literal + the inline openNewTaskModal call.
  const bind = rendererSrc.slice(rendererSrc.indexOf('if (status === TASKS_POST_PROCESSING_STATUS)'));
  const bindBody = bind.slice(0, bind.indexOf('header.appendChild(addBtn);'));
  assert.match(bindBody, /addBtn\.className\s*=\s*'tasks-lane-add'/);
  ...
  // And driving the REAL buildTasksLaneEl, the generated post-processing lane's Add
  // button click opens the new-ticket modal with status + kind post-processing.
  const mod = laneHarness.loadLaneModule(
    laneHarness.makeWindow().window, laneHarness.makeDocument(), console);
  const tab = laneHarness.makeTab({ config: null });
  mod.renderTasksBoard(tab);
  const ppLane = laneHarness.laneEls(tab).find((el) => el.dataset.status === 'post-processing');
  const addBtn = laneHarness.findByClass(ppLane, 'tasks-lane-add');
  assert.ok(addBtn, 'the generated post-processing lane owns an Add button');
...
test('Scenario (edge): editing a failed-testing ticket in the detail modal does not silently relabel it to todo', () => {
  // Given the detail-modal <select> is now built in JS from the folder's config
  // (TASK-102) — index.html ships it EMPTY — its options are the configured board
  // columns plus the "Won't do" pseudo-entry. With a null/absent config that is
  // exactly the six system lane statuses in order (still no failed-testing).
  assert.match(htmlSrc, /<select class="task-modal-status"><\/select>/,
    'the modal status select is an empty container the renderer fills (TASK-102)');
  ...
  mod.populateTaskStatusOptions(statusSel, mod.normalizeTasksColumns(null));
  const optValues = statusSel.children.map((o) => o.value);
  assert.ok(!optValues.includes('failed-testing'), 'no failed-testing option in the modal select');
  assert.ok(optValues.includes('post-processing'), 'post-processing option present');
...
function canonicalizeRouting(src) {
  return src
    ...
    // TASK-101 config-aware routing: the real renderer keys the user-column set as
    // `userStatuses`; the placeCard copy uses `USER_STATUSES`. Normalise both.
    .replace(/\buserStatuses\b/g, 'USERSET')
    .replace(/\bUSER_STATUSES\b/g, 'USERSET')
...
test('DRIFT GUARD: renderTasksBoard routing folds failed-testing into testing and matches the placeCard copy', () => {
  ...
  // TASK-101 made the `unknown` predicate config-aware (adds the user-column set
  // check), so the start marker tracks the new predicate.
  ...
  // Sanity: the canonical form is the expected config-aware routing (documents
  // intent + guards against a normalisation that accidentally erases the
  // behaviour). failed-testing still folds into testing; an out-of-config status
  // (neither valid nor a user column) routes to unknown, never silently to todo.
  assert.equal(
    canonicalizeRouting(routeSrc),
    "const unknown = !VALID.includes(S) && !USERSET.has(S); let laneKey; if (unknown) laneKey = UNKNOWN; "
...
test('DRIFT GUARD: the post-processing Add path composes status + kind post-processing and files into tasks/post-processing/', () => {
  ...
  // TASK-101: the Add affordance is a `<button class="tasks-lane-add">` built in
  // buildTasksLaneEl (className literal), no longer a `.tasks-lane-add` selector.
  assert.match(bind, /addBtn\.className\s*=\s*'tasks-lane-add'/, 'the Add affordance lives on the post-processing lane');
